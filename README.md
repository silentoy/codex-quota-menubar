...
- **按使用趋势**：结合最近历史消耗速度和距离重置的剩余时间判断瓶颈。5 小时额度参考最近 3 小时，周额度参考最近 24 小时；如果历史不足或没有实际消耗，会回退到静态支撑时间判断。

历史记录只保存在本机 `UserDefaults` 中，最多保留最近 24 小时的 50 条快照，仅用于本地瓶颈判断，不会额外上传。
